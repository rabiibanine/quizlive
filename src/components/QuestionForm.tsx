...
export default function QuestionForm({ onSave, onCancel }: QuestionFormProps) {
  ...
  const validate = (): boolean => {
    ...
    choices.forEach((choice, index) => {
      if (!choice.text.trim()) newErrors[`choice-${choice.id}`] = `Choice ${index + 1} is required`;
    });

    if (!correctChoiceId) newErrors.correctChoice = "Please select a correct answer";

    if (!Number.isFinite(time) || time < 1) newErrors.time = "Time must be at least 1 second";
  ...
  return (
    <div className="flex flex-col gap-6 p-6 rounded-lg border border-surface-border bg-surface-raised">
      ...
      <div className="flex flex-col gap-3">
        <span className="text-sm font-medium text-content-primary">Choices</span>
        ...
        {errors.correctChoice && (
          <span className="text-xs text-brand-danger">{errors.correctChoice}</span>
        )}
